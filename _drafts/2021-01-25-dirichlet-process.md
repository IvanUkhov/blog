...
$$n_1$$ observations $$x_1, \dots, x_{n_1}$$, the four parameters of the prior
are updated simultaneously (not sequentially) as follows:

$$
\begin{align}
\mu_0 & := \frac{\kappa_0}{\kappa_0 + n_1} \mu_0 + \frac{n_1}{\kappa_0 + n_1} \mu_x, \\
\kappa_0 & := \kappa_0 + n_1, \\
\nu_0 & := \nu_0 + n_1, \text{ and} \\
\sigma_0^2 & := \frac{1}{\nu_0 + n_1} \left( \nu_0 \sigma_0^2 + ss_x + \frac{\kappa_0 n_1}{\kappa_0 + n_1}(\mu_x - \mu_0)^2 \right)
\end{align}
$$

where $$\mu_x = \sum_{i = 1}^{n_1} x_i / n_1$$ and $$ss_x = \sum_{i = 1}^{n_1}
(x_i - \mu_x)^2$$.
