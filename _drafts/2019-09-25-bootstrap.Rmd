...
shall answer this question using only historical data and computer simulation
...
were impossible to perform before computers. To be able to answer such questions
back then, statisticians developed mathematical theories in order to approximate
the calculations for specific situations. These approximations and the various
terms and conditions under which they operate made up a large part of
traditional textbooks and courses in statistics, since nothing else was
possible. However, the advent of today’s computing power has made it possible to
...
is based on historical data and simulation rather than mathematical formulae
accompanied by stringent assumptions. The algorithm is straightforward from a
programmer’s perspective. It surely requires prior knowledge of the key concepts
in null hypothesis significance testing and statistics in general, such as false
positives, false negatives, sampling distributions, and critical values.
...
```{r sampling-distribution, include = FALSE}
library(tidyverse)

theme_set(theme_minimal(base_size = 14))

result <- simulate(sample_size, replication_count)
tibble(Null = result$difference_null,
       Alternative = result$difference_alternative) %>%
  gather(group, value) %>%
