...
It might be convenient to have one’s GUI preferences and alike (such as the font
size) be automatically set up upon each container launch. This can be achieved
by realizing that RStudio stores all user preferences in a folder called
`.rstudio`. Then the `start` command can be adjusted to silently plant a
preconfigured `.rstudio` into the current working directory, which can be seen
in the [repository][RStudio/] accompanying this article.
