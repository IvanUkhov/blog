...
what we are after, as they allow one to compute the false positive rate. First,
...
The final step is to put the above procedure in an optimization loop minimizing
the distance between the target and attained $$\beta$$’s with respect to the
sample size:
...
simulate <- function(sample_size, replication_count) {
  ...
  beta <- mean(delta_alternative < critical_value)
