...
Here, `Treatment sample` and `Control sample` stand for collections of
observations, which the performance metric is based on. For instance, each
observation could be the total deposit of a customer, and the metric is the
...
A sample corresponding to the status quo (that is, the current policy) can be
easily obtained by drawing $$n$$ samples with replacement from the data:
...
Large positive values of the test statistic speak in favor of the new policy,
while those that are close to zero suggest that the treatment is futile. The
goal of simulation is to obtain the distribution of the test statistic for two
cases: under the null hypothesis and under the alternative hypothesis. Given
these two distributions, which are referred to as sampling distributions, and
keeping $$\alpha$$ and $$\delta$$ set to the prescribed values, one can work out
the corresponding statistical power $$(1 - \beta)$$.
