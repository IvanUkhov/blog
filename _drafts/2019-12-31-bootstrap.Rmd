...
might reach for a thick book in statistics, match the situation at hand with a
statistical test, agree to the terms and conditions of the test, and proceed to
estimating the sample size using the corresponding formulae either manually or
via software packages. In what follows, we shall answer the question posed above
without any statistical formalism and any mathematical formulae. The only
...
campaigns will have a positive effect on a specific performance metric, such as
the average amount of deposits. In order to test the hypothesis, we decide to
perform an A/B test. There will be two groups: a control group and a treatment
group. The former will be exposed to the current promotion policy, while the
latter to the new one.

We have a level of statistical significance $$\alpha$$ and a level of practical
significance $$\delta$$ in mind. The former puts a limit on false positives, and
the latter indicates the smallest effect that we still care about; anything
smaller is as good as zero for any practical purpose. Furthermore, we require
the test to have a prescribed statistical power $$(1 - \beta)$$, which enforces
a constraint on false negatives.

Now, what should the size of each group be?
...
Suppose we have historical data $$\mathcal{D}$$ on the performance of the
current policy.
