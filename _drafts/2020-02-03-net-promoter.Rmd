...
and the one for promoters have the same structure, and for brevity, only the
former is described. For the log-odds of neutrals, the model is
...
rhstudent_t <- function(n, ...) {
  rtruncated(n,
             function(q) { pstudent_t(q, ...) },
             function(p) { qstudent_t(p, ...) })
}

set.seed(42)

n <- 10000
tibble(log_odds_ratio = rstudent_t(n, 5, 0, 1) +
...
# Generate some data (see the appendix)
data <- generate(N, n)
...
data$population %>%
  ...
  group_by(age, .draw) %>%
  summarize(score = sum(promoter - detractor) / sum(cell_size)) %>%
  tidybayes::mean_hdi()
```
