...
prior distribution of the noise, including Equations (2) and (3):
...
posterior predictive distribution of the response: for each draw, one simply has
to evaluate the mean vector and the covariance matrix and sample the
corresponding multivariate Gaussian distribution. The result is given in the
following figure:
...
It shows the mean value of the posterior predictive distribution given by the
black line along with a 95% equal-tailed probability band about the mean. It can
be seen that the uncertainty in the predictions is adequately captured along the
entire support.

Lastly, let us illustrate what would happen if the data were modeled as having
homogeneous noise. To this end, the variance of the noise is assumed to be
independent of the covariates. After repeating the inference and prediction
processes, the following is obtained:
...
* Mattias Villani, “[Advanced Bayesian learning][Villani 2020],” Stockholm
  University, 2020.
