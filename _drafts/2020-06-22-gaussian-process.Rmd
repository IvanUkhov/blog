...
directory <- '../_scripts/2020-06-22-gaussian-process'
source(paste0(directory, '/common.R'))
...
```{r, include = FALSE}
if (!file.exists('model-heteroscedastic.RData')) {
  model <- stan(file = paste0(directory, '/model-heteroscedastic.stan'),
                data = list(d = 1,
                            m = nrow(data),
                            x = as.matrix(data$x),
                            y = data$y))
  save(model, file = 'model-heteroscedastic.RData')
} else {
  load('model-heteroscedastic.RData')
}
print(model)
```
...
```{r posterior-predictive-noise, include = FALSE}
