...
```{r process-prior-short, include = FALSE}
process_prior_plot(ell_process = 0.1)
```

![](/assets/images/2020-06-22-gaussian-process/process-prior-short-1.svg)

With $$\ell_\text{process} = 0.5$$, the behavior changes to the following:

```{r process-prior-long, include = FALSE}
process_prior_plot(ell_process = 0.5)
```

![](/assets/images/2020-06-22-gaussian-process/process-prior-long-1.svg)
...
Based on the figures given above (but it should have been done prior to
collecting any data), the following is a reasonable configuration for the
parameters of the covariance function:

$$
\begin{align}
\sigma_\text{process} & \sim \text{half-Gaussian}\left( 0, 1 \right) \text{ and} \\
\ell_\text{process} & \sim \text{half-Gaussian}\left( 0, 1 \right).
\end{align}
$$

The notation means that it is the standard Gaussian distribution that is
truncated at zero and renormalized to yield a probability distribution. The
choice of priors is less straightforward in the case of the parameters of the
noise due to the link function. A solution is to make an assumption and run a
simulation to understand the consequences of the assumption. Consider the
following:
...
```{r noise-prior, include = FALSE}
noise_prior_plot(sigma = 0.1)
```

![](/assets/images/2020-06-22-gaussian-process/noise-prior-1.svg)

It can be seen that the priors allow for substantial noise, considering the
scale of the response; see the very first figure. Yet, the draws are not utterly
unrealistic. Therefore, the priors are adequate.
