...
process_prior_plot(ell_process = 0.1)
```
...
process_prior_plot(ell_process = 0.5)
```
...
```{r prior-length-scale, include = FALSE}
...
![](/assets/images/2020-06-22-gaussian-process/prior-length-scale-1.svg)
...
Next, the choice of priors for the parameters of the noise is complicated by the
nonlinear link function. A solution is to make an assumption and perform a prior
predictive check to understand the consequences of the assumption. Consider the
following:

$$
\begin{align}
\alpha_\text{noise} & \sim \text{Gaussian}\left( 0, 0.1 \right) \text{ and} \\
\beta_{\text{noise}, i} & \sim \text{Gaussian}\left( 0, 0.1 \right),
\text{ for } i = 1, \dots, d.\\
\end{align}
$$

Here are 10 draws from these priors, including Equations (2) and (3):

```{r prior-noise, include = FALSE}
noise_prior_plot(sigma = 0.1)
```

![](/assets/images/2020-06-22-gaussian-process/prior-noise-1.svg)

It can be seen that the priors allow for substantial noise, considering the
scale of the response. Yet, the draws are not utterly unrealistic. Therefore,
the priors are adequate.
...
\alpha_\text{noise}
& \sim \text{Gaussian}\left( 0, 0.1 \right); \text{ and} \\

\beta_{\text{noise}, i}
& \sim \text{Gaussian}\left( 0, 0.1 \right),
\text{ for } i = 1, \dots, d.\\
\end{align}
$$
