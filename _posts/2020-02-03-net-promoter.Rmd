...
```{r prior-distribution, include = FALSE}
rtruncated <- function(n, distribution, inverse, quantile = 0) {
  inverse(runif(n, min = distribution(quantile), max = 1))
}

rhstudent_t <- function(n, ...) {
  rtruncated(n,
             function(q) { brms::pstudent_t(q, ...) },
             function(p) { brms::qstudent_t(p, ...) })
}

set.seed(42)

n <- 10000
tibble(log_odds_ratio = brms::rstudent_t(n, 5, 0, 1) +
                        rnorm(n, sd = rhstudent_t(n, 5, 0, 1)) +
                        rnorm(n, sd = rhstudent_t(n, 5, 0, 1))) %>%
  filter(log_odds_ratio > quantile(log_odds_ratio, 0.005),
         log_odds_ratio < quantile(log_odds_ratio, 0.995)) %>%
  ggplot(aes(log_odds_ratio)) +
  stat_density(geom = 'line', size = 1) +
  geom_vline(xintercept = 5, linetype = 'dashed') +
  geom_vline(xintercept = -5, linetype = 'dashed') +
  scale_x_continuous(breaks = seq(-10, 10), minor_breaks = NULL) +
  labs(x = 'Log-odds of neutrals with respect to detractors',
...
where $$I^i_j$$ is as in Equation 3, for $$i = 1, \dots, M$$ and $$j = 1, \dots,
m_i$$. The above gives a poststratified draw from the posterior distribution of
the net promoter score for variable $$i$$ and value $$j$$. In practice,
depending on the tool used, one might perform the poststratification procedure
