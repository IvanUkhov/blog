...
if (!file.exists(cache_path)) {
  draws <- sample_DPM(data$x, 25, 10000, mu0 = 20, kappa0 = 0.01)
...
```{r mixture-posterior-mu, fig.asp = 3, include = FALSE}
burnin_proportion <- 0.1
component_count = 10

tail(draws, (1 - burnin_proportion) * length(draws)) %>%
...
if (!file.exists(cache_path)) {
  draws_summarized <- summarize_inference(
    tail(draws, (1 - burnin_proportion) * length(draws)), data
  )
