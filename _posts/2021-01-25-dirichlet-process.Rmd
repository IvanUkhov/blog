...
sample_Ptheta_prior(1000, mu0 = 20, kappa0 = 0.01) %>%
  ggplot() +
...
if (!file.exists(cache_path)) {
  draws0 <- sample_DPM(data$x, 25, 2000, mu0 = 20, kappa0 = 0.01, prior_only = TRUE)
...
if (!file.exists(cache_path)) {
  draws <- sample_DPM(data$x, 25, 2000, mu0 = 20, kappa0 = 0.01)
...
```{r mixture-posterior-mu, fig.asp = 2, include = FALSE}
tail(draws, length(draws) / 2) %>%
  sapply(function(draw) draw$theta$mu[1:7], simplify = FALSE) %>%
  { do.call(rbind, .) } %>%
  as_tibble() %>%
  rename_all(function(name) gsub('V', 'Component ', name)) %>%
  ...
  geom_vline(aes(xintercept = 24.5, color = 'Observation'), size = 1) +
  geom_vline(aes(xintercept = 26, color = 'Observation'), size = 1) +
